...
class Ui_Pref(QWidget):
	...
	def __init__(self,loadTab,pref,mainW,parent=None):
	...
	def checkSys(self,value):
		if os.path.isfile("/usr/bin/crontab") and os.path.isfile("/usr/bin/sudo"):
			if value: #Enable sys button
				...
				if self.pref.startWithSystem: #Disable cron
	...
	def checkTime(self,value):
		if self.mainWindows.setAutoUpdate():
			...
			self.pref.autoUpdate = value
			if self.pref.updateTime <= 1:
	...
	def disableCronStartup(self):
		...
		if int(result) == 0:
			...
			self.pref.valueStart = "0:0"
			self.mainWindows.setStartSystem(self.pref.startWithSystem,self.pref.valueStart)
	...
	def enableCronStartup(self):
		...
		if int(result) == 0:
			...
			self.pref.valueStart = str(offsetGpu) + ":" + str(offsetMem)
			self.mainWindows.setStartSystem(self.pref.startWithSystem,self.pref.valueStart)
			self.mainWindows.saveNvidiuxConf()
			...
			if self.pref.nbGpuNvidia == len(listgpu):
				try:
					for tempgpu in listgpu:
						if str(self.pref.gpu[i].nameGpu) != str(tempgpu[0]):
							errorCode = 12
						if int(tempgpu[1]) < int((self.pref.gpu[i].defaultFreqGpu)) * 0.80 or int(tempgpu[1]) > int((self.pref.gpu[i].defaultFreqGpu)) * 1.3:
							errorCode = 13
						if int(tempgpu[2]) < int((self.pref.gpu[i].defaultFreqShader)) * 0.80 or int(tempgpu[2]) > int((self.pref.gpu[i].defaultFreqShader)) * 1.3:
							errorCode = 14
						if int(tempgpu[3]) < int((self.pref.gpu[i].defaultFreqMem)) * 0.80 or int(tempgpu[3]) > int((self.pref.gpu[i].defaultFreqMem)) * 1.3:
							errorCode = 15
						if int(tempgpu[4]) < 0 or int(tempgpu[4]) > self.pref.gpu[i].maxOvervolt:
							errorCode = 15
						i = i + 1
				except:
	...
	def saveMonitorConf(self):
		...
		text = fileToSave.createTextNode(self.pref.nvidiuxVersionStr)
	...
	def setLanguage(self,lang):
		...
		if prefTranslator.load("/usr/share/nvidiux/nvidiux_" + language):
			self.pref.app.installTranslator(prefTranslator)
			self.mainWindows.setTimeUpdate(self.pref.updateTime)
			self.retranslateUi()
	...
	def setupUi(self):
		...
		self.spinBox.setEnabled(self.pref.autoUpdate)
		self.spinBox.setValue(self.pref.updateTime)
		if self.pref.updateTime <= 1:
			self.spinBox.setSuffix(_translate("Form", " seconde", None))
		else:
			self.spinBox.setSuffix(_translate("Form", " secondes", None))
		...
		elif self.pref.language == "de_DE":
			self.combolang.setCurrentIndex(2)
		elif self.pref.language == "es_ES":
		...
		self.checkBoxOverVolt.setChecked(False)
		if self.pref.piloteVersion >= 346.16 and self.pref.overclockEnabled:
			overvolt = True
		...
		self.checkBoxUpdateMon.setChecked(self.autoUpdate)
		self.groupBoxPrefGpu = QtGui.QGroupBox(self.tabMoniteur)
		self.groupBoxPrefGpu.setGeometry(QtCore.QRect(10, 50, 220, 50 * self.pref.nbGpuNvidia + 5 ))
		...
		for i in range(0, self.pref.nbGpuNvidia):
			...
			self.listLabelGpu[i].setText(str(i + 1) + ":" + str(self.pref.gpu[i].nameGpu))
		...
		self.checkBoxChangeGenMon.setGeometry(QtCore.QRect(10, 70 + self.pref.nbGpuNvidia * 50, 340, 20))
